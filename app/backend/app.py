...
@app.route("/getstatusupdates", methods=["POST"])
def get_status_updates():
    """Get all status updates for file with matching id"""
    try:
        id = request.json["id"]
        results = statusLog.read_file_status(id)
    except Exception as ex:
        logging.exception("Exception in /getstatusupdates")
        return jsonify({"error": str(ex)}), 500
    return jsonify(results)
